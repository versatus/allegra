...
use allegra::helpers::{
    load_bootstrap_node, load_or_create_ethereum_address, load_or_get_public_ip_addresss,
};
use allegra::publish::{GenericPublisher, NetworkTopic, QuorumTopic};
#[allow(unused)]
use allegra::statics::{
    DEFAULT_GRPC_ADDRESS, DEFAULT_LXD_STORAGE_POOL, DEFAULT_PUBLISHER_ADDRESS,
...
async fn main() -> std::io::Result<()> {
    ...
    let lxd_network_interface =
        std::env::var("LXD_NETWORK_INTERFACE").unwrap_or(DEFAULT_NETWORK.to_string());
    ...
    let vmmserver = VmmServer::new(service);
    log::info!("created vmm server");

    let addr = std::env::var("GRPC_ADDRESS")
        .unwrap_or(DEFAULT_GRPC_ADDRESS.to_string())
        .parse()
    ...
    let to_dial = if let Ok(bootstrap_nodes) = load_bootstrap_node(None).await {
        bootstrap_nodes
            ...
            .filter_map(|(wallet, ip)| {
                let bootstrap_peer = Peer::new(wallet.clone(), ip.clone());

                if bootstrap_peer != local_peer {
                    log::warn!(
                        "bootstrap peer: {:?} is not local peer, dialing...",
                        &bootstrap_peer
                    );
                    Some(bootstrap_peer)
                } else {
