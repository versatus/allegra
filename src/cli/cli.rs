...
use allegra::cli::helpers::{create_allegra_rpc_client, generate_signature_from_command};
use clap::Parser;
...
#[command(
    about = "enables the provisioning, starting, stopping, and deleting of VMs in the Versatus Network"
)]
struct Cli {
    #[command(subcommand)]
    ...
    match &cli.command {
        AllegraCommands::Wallet {
            ...
            let wallet = if *new {
                ...
                let wallet_info: WalletInfo = serde_json::from_slice(&buffer)
                    .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        ...
        AllegraCommands::Stop { name, endpoint, .. } => {
        ...
        AllegraCommands::ExposeService {
            ...
            let service_type: Vec<i32> =
                service_type.par_iter().map(|s| s.clone().into()).collect();
            let (sig, recover_id) = generate_signature_from_command(cli.command.clone())?;
            let recovery_id: u8 = recover_id.into();
