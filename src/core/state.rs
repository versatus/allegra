...
use tikv_client::{
    ...
    Backoff, BoundRange, ColumnFamily, Config, Key, KvPair, RawClient, Result, Snapshot, Timestamp,
...
use crate::{
    ...
    event::{GeneralResponseEvent, StateEvent, StateValueType, TaskStatusEvent},
    params::ServiceType,
    publish::{GenericPublisher, StateTopic},
    subscribe::{StateSubscriber, TaskStatusSubscriber},
...
impl StateManager {
    pub async fn new(
        ...
        let writer = StateWriter::new(pd_endpoints.clone())
            .await
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
        log::info!("Created StateWriter...");
        let reader = StateReader::new(pd_endpoints.clone())
            .await
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
        ...
        let task_cache = Arc::new(RwLock::new(LruCache::new(NonZeroUsize::new(1000).ok_or(
            std::io::Error::new(std::io::ErrorKind::Other, "Invalid NonZeroUsize"),
        )?)));
        log::info!("Created TaskCache...");
        let account_cache = Arc::new(RwLock::new(LruCache::new(NonZeroUsize::new(1000).ok_or(
            std::io::Error::new(std::io::ErrorKind::Other, "Invalid NonZeroUsize"),
        )?)));
        log::info!("Created AccountCache...");
        let instance_cache = Arc::new(RwLock::new(LruCache::new(NonZeroUsize::new(1000).ok_or(
            std::io::Error::new(std::io::ErrorKind::Other, "Invalid NonZeroUsize"),
        )?)));
    ...
    async fn update_or_create_instance(
        ...
        let port_map_iter: Vec<(u16, (u16, ServiceType))> = port_map
            .par_iter()
            .map(|(x, (y, st))| (*x, (*y, st.clone().into())))
            .collect();
        if let Some(instance_bytes) = self
            .reader_mut()
            .get(
            ...
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?
        {
            ...
            let instance = Instance::new(namespace, vminfo, port_map, last_snapshot, last_sync);
    ...
    async fn handle_state_event(&mut self, m: StateEvent) -> std::io::Result<()> {
        ...
        match m {
            ...
            StateEvent::PutAccount {
                ...
                self.writer_mut()
                    .put_optimistic(owner.to_vec(), serde_json::to_vec(&account)?)
                    .await
                    .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
            }
            ...
            StateEvent::Get {
                ...
                let response_event = match expected_type {
                    StateValueType::Account => {
                        let account: Account = serde_json::from_slice(&data_bytes)?;
                    ...
                    StateValueType::TaskStatus => {
                        let task_status: TaskStatus = serde_json::from_slice(&data_bytes)?;
                        GeneralResponseEvent::new(
                            response_event_id,
                            event_id.clone(),
                    ...
                    StateValueType::Instance => {
                        let instance: Instance = serde_json::from_slice(&data_bytes)?;
                        GeneralResponseEvent::new(
                            response_event_id,
                            event_id.clone(),
                ...
                for response_topic in response_topics {
                    guard
                        .publish(Box::new(response_topic), Box::new(response_event.clone()))
                        .await?;
                }
            ...
            StateEvent::GetAccount {
                ...
                for topic in response_topics {
                    guard
                        .publish(Box::new(topic), Box::new(response_event.clone()))
                        .await?;
                }
            }
            StateEvent::GetInstance {
                event_id,
                task_id,
                task_status,
                namespace,
                ...
                let data_bytes = self
                    .reader_mut()
                    .get(serde_json::to_vec(&namespace)?)
                    .await
                    .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?
                    .ok_or(std::io::Error::new(
                        std::io::ErrorKind::Other,
                        format!(
                            "unable to find key 0x{} in state db",
                            hex::encode(&serde_json::to_vec(&namespace)?)
                        ),
                    ))?;
            ...
            StateEvent::GetTaskStatus {
                ...
                let data_bytes = self
                    ...
                    .ok_or(std::io::Error::new(
                        ...
                        format!(
                            "unable to find key 0x{} in state db",
                            hex::encode(&serde_json::to_vec(&task_id)?)
                        ),
                    ))?;

                let task_status = serde_json::from_slice(&data_bytes)?;
                let response_event_id = Uuid::new_v4().to_string();
            ...
            StateEvent::DeleteInstance {
                ...
                self.writer_mut()
                    .delete(serde_json::to_vec(&namespace)?)
                    .await
                    .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
            }
            StateEvent::DeleteTaskStatus { event_id, task_id } => {
                self.writer_mut()
                    .delete(serde_json::to_vec(&task_id)?)
                    .await
                    .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
            }
            StateEvent::Delete {
        ...
        match task_event {
            TaskStatusEvent::Get {
                ...
                if let Some(task_status) = read_guard.get(&original_task_id) {
                    ...
                    for topic in response_topics {
                        publish_guard
                            .publish(Box::new(topic), Box::new(response.clone()))
                            .await?;
                    }
                    drop(publish_guard);
                }
            ...
            TaskStatusEvent::Update {
                ...
                *entry = task_status.clone();
                let mut guard = self.publisher().lock().await;
                let event = StateEvent::PutTaskStatus {
                    event_id: new_event_id,
                    task_id,
                    task_status,
                };
                guard.publish(Box::new(StateTopic), Box::new(event)).await?;
            }
        }
...
impl StateWriter {
    ...
    pub async fn new_with_config<'a>(pd_endpoints: Vec<&'a str>, config: Config) -> Result<Self> {
        let client = TransactionClient::new_with_config(pd_endpoints, config).await?;
        Ok(Self { client })
    }

    pub async fn put_optimistic(&self, k: impl Into<Key>, v: impl Into<Value>) -> Result<()> {
    ...
    pub async fn put_pessimistic(&self, k: impl Into<Key>, v: impl Into<Value>) -> Result<()> {
    ...
    pub async fn batch_put(&self, kv_pairs: impl ParallelIterator<Item = KvPair>) -> Result<()> {
        let mut txn = self.client.begin_optimistic().await?;
        let mutations: Vec<Mutation> = kv_pairs
            .into_par_iter()
            .map(|kv| {
    ...
    pub async fn garbage_collect(&self, safepoint: Timestamp) -> Result<bool> {
        self.client.gc(safepoint).await
    }

    pub async fn delete(&self, key: impl Into<Key> + Clone) -> Result<()> {
        let mut txn = self.client.begin_optimistic().await?;
...
impl StateReader {
    ...
    pub fn new_with_column_family(self, cf: ColumnFamily) -> Self {
    ...
    pub async fn batch_scan(
        ...
        self.client.batch_scan(ranges, each_limit).await
    }
    ...
    pub async fn batch_scan_keys(
        ...
        self.client.batch_scan_keys(ranges, each_limit).await
    }
    ...
    pub async fn scan_keys(&self, range: impl Into<BoundRange>, limit: u32) -> Result<Vec<Key>> {
        self.client.scan_keys(range, limit).await
    }
}
